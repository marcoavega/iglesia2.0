...
if (empty($year)) {
	...
	<div class="col-md-12 home">
		<form method="POST" class="">
			<h3 class="centrartexto padding-versiculo">No hay Años</h3>
		</form>
	</div>
</div>
<script>
...
if (empty($trimester)) {
	print('
		<div class="">
			<p class="item  error">No hay Trimestres</p>
		</div>
	');
} else {
	print('
...
if (empty($date)) {
	print('
		<div class="">
			<p class="item  error">No hay fechas</p>
		</div>
	');
} else {
	$template_date = '
...
if (empty($name_lesson)) {
	print('
		<div class="">
			<p class="item  error">No hay Títulos de lecciones</p>
		</div>
	');
} else {
...
if (empty($objective)) {
	print('
		<div class="">
			<p class="item  error">No hay objetivos de lecciones</p>
		</div>
	');
} else {
	$template_objective = '
...
if (empty($title_verse)) {
	print('
		<div class="">
			<p class="item  error">No hay títulos de versos</p>
		</div>
	');
} else {
...
if (empty($verse_text)) {
	print('
		<div class="">
			<p class="item  error">No hay textos de versos</p>
		</div>
	');
} else {
	$template_verse_text = '
...
if (empty($comment)) {
	print('
		<div class="">
			<p class="item  error">No hay textos de comentarios</p>
		</div>
	');
} else {
	$template_comment = '
...
if (empty($bible_reading)) {
	print('
		<div class="">
			<p class="item  error">No hay textos de lecturas bíblicas</p>
		</div>
	');
} else {
...
for ($q = 1; $q <= 8; $q++) {
	...
	if (empty($question)) {
		print('
		<div class="">
			<p class="item  error">No hay preguntas ' . $q . '</p>
		</div>
			<tr>
				<th colspan="2">
					<form method="POST">
...
for ($b = 1; $b <= 8; $b++) {
	...
	if (empty($quote)) {
		print('
		<div class="">
			<p class="item  error">No hay títulos de citas ' . $b . '</p>
		</div>
			<tr>
				<th colspan="2">
					<form method="POST">
						<input type="hidden" name="r" value="capture-lessons-add-quote' . $b . '">
						<input class="" type="submit" value="Agregar cita">
					</form>
				</th>
			</tr>
	');
...
for ($c = 1; $c <= 8; $c++) {
	...
	if (empty($bquote)) {
		print('
		<div class="">
			<p class="item  error">No hay citas ' . $c . '</p>
		</div>
			<tr>
				<th colspan="2">
					<form method="POST">
